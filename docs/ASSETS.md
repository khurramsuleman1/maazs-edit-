...
|---|---|---|---|---|
| Shared architecture (Blender reference; D57 web override = tall wall + floor only) | APPROVED | collection `BA_SW_REAL_RATIO_REBUILD`; web `GalleryScene.createArchitecture()` | not exported; code-native runtime | Blender source remains unchanged. Master Khurram directed the 2026-07-22 web-only simplification: one 18×7.2 textured plaster wall plus the existing black-gold floor on desktop/mobile; no new authored asset or gate transition. |
